...
public class MergeSort {  
   ...
   * @return String 
   */  
  private static OrderByDto nextLine(LineIterator iterator) {
    if(iterator.hasNext()) {
  ...
  private static void split(String fileA, String fileB, String fileC, int k,  
    ...
    List<OrderByDto> bufList = new ArrayList<OrderByDto>(blockSize); //大小为blockSize的缓冲区
    Writer fA = null;  
    Writer fB = null;  
    Reader fC = null;  
    try {  
  ...
  public static void sort(String fileSource,String fileOut, int blockSize,boolean removeDuple) throws IOException {  
    String fileA = File.createTempFile("wjw", null,new File("/Users/zenghuikang/workspace/coinflex/atguigu/flink/UserBehaviorAnalysis/HotItemsAnalysis/src/main/resources/")).getAbsolutePath();
    String fileB = File.createTempFile("wjw", null,new File("/Users/zenghuikang/workspace/coinflex/atguigu/flink/UserBehaviorAnalysis/HotItemsAnalysis/src/main/resources/")).getAbsolutePath();
    ...
    try {  
      ...
      if(lineSize <= blockSize) { //对于小文件,从fC读入数据,直接排序后写回文件中
        ...
        IOUtils.closeQuietly(fC);  
        FileUtils.writeLines(new File(fileOut), "UTF-8", list, "\n");
